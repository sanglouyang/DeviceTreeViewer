<template>
    <div class="can-card">
        <div class="can-header" @click="toggle">
            <div class="can-title">
                <span class="can-toggle" :class="{ collapsed: !isExpanded }">
                    <svg viewBox="0 0 24 24" width="12" height="12">
                        <path fill="currentColor" d="M8 5l8 7-8 7z"/>
                    </svg>
                </span>
                <span class="can-icon">🔗</span>
                <span class="can-name">{{ node.name }}</span>
                <span v-if="node.label" class="can-label">{{ formatLabel(node.label) }}</span>
                <span class="can-status" :class="statusClass(node.properties.status)">
                    {{ statusText(node.properties.status) }}
                </span>
                <span v-if="pinctrlValues.length > 0" class="can-pinctrl-group">
                    <span v-for="pinctrl in pinctrlValues" :key="pinctrl" class="can-pinctrl">
                        {{ pinctrl }}
                    </span>
                </span>
            </div>
        ...
        <div v-show="isExpanded" class="can-body">
            <div class="can-props">
...
.can-toggle.collapsed {
    transform: rotate(0deg);
}

.can-toggle:not(.collapsed) {
